if defined?(Merb::Plugins)
  ...
  Merb::Slices::config[:quickadmin][:layout] ||= :quickadmin
  
  # All Slice code is expected to be namespaced inside a module
  module Quickadmin
    
    # Slice metadata
    self.description = "Quickadmin is a chunky Merb slice!"
    self.version = "0.0.1"
    ...
    def self.setup_router(scope)
      ...
      # the slice is mounted at /quickadmin - note that it comes before default_routes
      scope.match('/').to(:controller => 'main', :action => 'index').name(:home)
      scope.match('/').to(:controller => 'validates', :action => '').name(:home)
